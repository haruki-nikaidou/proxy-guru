...
{:else}
	<!-- A feed that failed after it had events keeps showing them: say they may
	     be out of date instead of letting the table freeze silently. -->
	{#if feeds.error}
		<BoundaryError error={feeds.error} retry variant="inline" title={m.error_stale_title()} />
	{/if}
