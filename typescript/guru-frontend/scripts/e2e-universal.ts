...
if (!email || !password || !ingressName || !exitDestination || transitNames.length === 0) {
	console.error(
		'usage: e2e-universal.ts <email> <password> <ingress> <exit host:port> <transit>...'
	);
	process.exit(2);
}
...
const canvas = (
	await orch.createCanvas(
		{ name: `universal ${new Date().toISOString()}`, description: 'universal-node e2e' },
		opts
	)
).canvas!;
...
const connect = (outputPortId: string, inputPortId: string) =>
	orch.connectPorts({ outputPortId, inputPortId }, opts);
...
for (const [i, p] of [10000, 10001].entries()) {
	const pod = await node(
		`ingress-${p}`,
		{ pod: { serverId: ingress.id, port: p, bindIp: '', advertiseIp: '' } },
		0,
		0
	);
	...
	await orch.connectPorts(
		{
			outputPortId: '',
			inputPortId: port(pod, 'destination'),
			outputHandle: handle(ud.id, UniversalGroup.CHANNEL_OUT)
		},
		opts
	);
	pods.push(pod);
...
for (const [i, t] of transits.entries()) {
	...
	await orch.connectPorts(
		{
			outputPortId: port(ud, `member_${i + 1}`),
			inputPortId: '',
			inputHandle: handle(up.id, UniversalGroup.BUNDLE_IN)
		},
		opts
	);
	await orch.connectPorts(
		{ outputPortId: port(up, 'bundle_out'), inputPortId: port(ua, `member_${i + 1}`) },
		opts
	);
}
const detail = await orch.getCanvas({ canvasId: canvas.id }, opts);
