...
const login = await auth.login({ email: email!, password: password!, userAgent: 'e2e-reroll' });
...
const used = new Set(detail.nodes.filter(n => n.spec?.pod?.serverId === server.id).map(n => n.spec!.pod!.port));
const [low, high] = [Number(lo), Number(hi)];
const free = () => { let p; do { p = 40000 + Math.floor(Math.random() * 20000); } while ((p >= low && p <= high) || used.has(p)); used.add(p); return p; };
for (const node of detail.nodes) {
	const pod = node.spec?.pod;
	if (!pod || pod.serverId !== server.id || pod.port < low || pod.port > high) continue;
	const port = free();
	await orch.replaceNodeSpec({ nodeId: node.id, spec: { pod: { ...pod, port } }, itemCount: 0 }, opts);
