...
if (action === 'protocol') {
	const ud = detail.nodes.find(
		n => n.spec?.loadBalanceDistribute && n.ports.some(p => p.key.startsWith('chan:'))
	)!;
	const cfg = ud.spec!.loadBalanceDistribute!;
	const protocol =
		arg === 'quic'
			? RelayProtocol.RELAY_QUIC
			: arg === 'tcp_tls'
				? RelayProtocol.RELAY_TCP_TLS
				: RelayProtocol.RELAY_TCP_RAW;
	await orch.replaceNodeSpec(
		{ nodeId: ud.id, spec: { loadBalanceDistribute: { ...cfg, protocol } }, itemCount: 0 },
		opts
	);
	...
	for (const edge of detail.edges)
		if (ins.has(edge.targetPortId)) await orch.disconnect({ edgeId: edge.id }, opts);
	...
	const members = (arg2 ?? '')
		.split(',')
		.map((name, i) => ({ slot: current[i]?.slot ?? ++next, name }));
	...
	const ua = detail.nodes.find(
		n => n.spec?.loadBalanceAggregate && !n.lane && n.ports.some(p => p.key === `chan:${pod.id}`)
	)!;
	const chan = ua.ports.find(p => p.key === `chan:${pod.id}`)!;
	await orch.connectPorts(
		{ outputPortId: exit.ports.find(p => p.key === 'destination')!.id, inputPortId: chan.id },
		opts
	);
}
const after = await orch.getCanvas({ canvasId: canvasId! }, opts);
const lanes = after.nodes
	.filter(n => n.lane)
	.map(
		n =>
			`${n.lane!.role}:${n.name}${n.spec?.pod ? ':' + n.spec.pod.port : ''}${n.spec?.relay ? ':' + RelayProtocol[n.spec.relay.protocol] : ''}`
	)
	.sort();
const members = after.nodes
	...
	.map(
		n =>
			`${n.name}: ${(n.spec?.loadBalanceDistribute?.members ?? n.spec?.loadBalanceAggregate?.members ?? []).map(m => `${m.slot}=${m.name}`).join(' ')}`
	);
console.log(
	JSON.stringify(
		{
			members,
			lanes,
			problems: (await orch.validateCanvas({ canvasId: canvasId! }, opts)).problems.map(
				p => p.message
			)
		},
		null,
