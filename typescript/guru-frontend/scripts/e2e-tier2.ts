...
const bundle = (outputPortId: string, to: string) =>
	orch.connectPorts(
		{ outputPortId, inputPortId: '', inputHandle: handle(to, UniversalGroup.BUNDLE_IN) },
		opts
	);
...
const upOf = (name: string) =>
	detail.nodes.find(n => n.spec?.universalPod?.serverId === server(name).id)!;
const byName = (name: string) => detail.nodes.find(n => n.name === name)!;
const us = server('us-ingress');
const ingress = detail.nodes.find(
	n => n.spec?.pod?.serverId === us.id && n.name.startsWith('ingress-')
)!;
const y = Number(ingress.position?.y ?? 0n);

// 1. Thin line: ingress-10002 straight into hk-1.
const p2 = (
	await orch.createNode(
		{
			canvasId: canvasId!,
			name: 'ingress-10002',
			comment: '',
			spec: { pod: { serverId: us.id, port: 10002, bindIp: '', advertiseIp: '' } },
			position: { x: 0n, y: 0n },
			itemCount: 0
		},
		opts
	...
	await orch.createNode(
		{
			...
			name: 'entry-10002',
			comment: '',
			spec: { entry: { tls: undefined } },
			position: { x: -350n, y: BigInt(y + 320) },
			itemCount: 0
		},
		opts
	)
...
await orch.connectPorts(
	{ outputPortId: port(p2, 'listen'), inputPortId: port(entry, 'listen') },
	opts
);
await orch.connectPorts(
	{
		outputPortId: '',
		inputPortId: port(p2, 'destination'),
		outputHandle: handle(upOf('hk-1').id, UniversalGroup.CHANNEL_OUT)
	},
	opts
);
	...
	await orch.createNode(
		{
			...
			position: { x: 1350n, y: BigInt(y + 380) },
			itemCount: 0
		},
		opts
	)
	...
	await orch.createNode(
		{
			...
			name: 'exit-2',
			comment: '',
			spec: { exit: { destination: exitDestination! } },
			position: { x: 1700n, y: BigInt(y + 320) },
			itemCount: 0
		},
		opts
	)
...
await orch.connectPorts(
	{ outputPortId: port(exit2, 'destination'), inputPortId: port(join, `chan:${p2.id}`) },
	opts
);
const problems = (await orch.validateCanvas({ canvasId: canvasId! }, opts)).problems.map(
	p => p.message
);
detail = await canvas();
