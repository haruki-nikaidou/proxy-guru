...
- **`entities/db`：** 每个表或聚合对应一个子模块；行结构体派生 `sqlx::FromRow`；id 由
  ...
  `begin()` 事务。查询是在运行时校验的，因此必须由针对真实数据库的集成测试覆盖（`#[sqlx::test]`）。
...
- `sqlx::query*` 接受的是 `&'static str`，所以在运行时拼出来的语句需要 `sqlx::AssertSqlSafe` ——
  只在测试里这么做；`src/` 中没有任何地方格式化 SQL。
