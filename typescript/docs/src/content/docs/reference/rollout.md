...
(the health stream closed, or no report for three intervals — the cron sweep). Both histories are
raw and trimmed by the retention cron; `ListServerHealthHistory` / `ListNodeHealthHistory` read a
time range.
...
`<pod-key>.relay.guru.internal`), the cron rotates leaves ten days before expiry, and workers verify
relay peers only against `certs/ca.pem` (`relay_ca` in the TOML). DNS providers, certificates and
the CA are Admin-managed through the operator API (`CreateDnsProvider`, `ListCertificates`,
`RetryCertificate`, …); secrets are encrypted with `GURU_MASTER_KEY` and never returned.
