...
- **`entities/db`:** one submodule per table or aggregate; a row struct deriving `sqlx::FromRow`;
  the id wrapped by `table_record!(NameId, "table")`, an enum stored as text spelled once by
  `text_enum!`. One `Processor` impl per query on `base::db::Db`, with `Error = base::db::Error`;
  a multi-statement write is one `begin()` transaction. Queries are checked at runtime, so they are
  covered by integration tests against a real database (`#[sqlx::test]`).
...
- `sqlx::query*` take a `&'static str`, so a statement built at runtime needs
  `sqlx::AssertSqlSafe` — tests only; nothing in `src/` formats SQL.
