...
Every statement goes through sqlx's `query!` family, so the SQL is checked against a real schema
while the crate compiles. A normal build needs no database: it reads the offline data committed in
`.sqlx/`. After adding or editing a statement (or a migration), regenerate that data against a
migrated database and commit it, or a build without `DATABASE_URL` will still see the old query set:

```sh
export DATABASE_URL=postgres://guru:guru@127.0.0.1:15432/guru_dev
# manage-tool reads GURU_DATABASE_URL, not DATABASE_URL — pass it explicitly
cargo run -p manage-tool -- --database-url "$DATABASE_URL" db migrate
cargo sqlx prepare --workspace -- --all-targets   # rewrites .sqlx/
```

`cargo sqlx prepare` needs `sqlx-cli` (`cargo install sqlx-cli --no-default-features --features
postgres,rustls`). `-- --all-targets` is not optional: without it the tests' own statements are left
out of `.sqlx/` and an offline `cargo test` fails on them. When `DATABASE_URL` is exported, the
macros talk to that database directly and `.sqlx/` is ignored — which is why the regenerated data
has to be committed separately.
...
Create it once with `createdb` (or `CREATE DATABASE guru_test;`); the role needs `CREATEDB`. The
runner creates and migrates a sibling database per test, so the database `DATABASE_URL` names never
needs the schema itself — but the `query!` macros would try to check every statement against it and
fail on the missing tables. `SQLX_OFFLINE=true` points them at `.sqlx/` instead, which is what keeps
the two roles of `DATABASE_URL` apart. (Migrating that database once works too.)
