...
|---|---|---|
| 設定の正となる場所 | ノード上のファイル | SurrealDB 内のキャンバス |
| SurrealDB / RabbitMQ / マスターが必要か | 不要 | 必要 |
...
2. 2 つのファイルを比較します。`manage-tool orchestration export-config --server <key>` は、キャンバスが現在導出している内容、つまりマスターがストリーム配信するはずのファイルを出力します。（このコマンドは SurrealDB と通信するので、ワーカーノードではなくオペレーターのマシンで実行します。）
