...
- **`entities/db`:** テーブルまたは集約ごとに 1 サブモジュール。行構造体は `sqlx::FromRow` を derive し、ID は `table_record!(NameId, "table")` でラップします。テキストとして保存する enum は `text_enum!` で一度だけ書きます。クエリごとに `base::db::Db` 上の `Processor` 実装を 1 つ用意し、`Error = base::db::Error` とします。複数ステートメントの書き込みは 1 つの `begin()` トランザクションにまとめます。クエリは実行時に検査されるため、実データベースに対する結合テスト（`#[sqlx::test]`）でカバーします。
...
- `sqlx::query*` は `&'static str` を取るため、実行時に組み立てたステートメントには `sqlx::AssertSqlSafe` が必要です — これはテストのみで、`src/` 内で SQL を文字列整形している箇所はありません。
