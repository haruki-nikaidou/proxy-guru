...
pub(crate) fn port_rows(owner: &NodeId, existing: &[PortEntity], ports: &[NewPort]) -> Vec<PortEntity> {
...
fn ensure_lane_edit_allowed(old: &NodeWithPorts, spec: &NodeSpec) -> Result<(), OrchestrationError> {
...
fn ensure_retire_allowed(topology: &CanvasTopology, node: &NodeEntity) -> Result<(), OrchestrationError> {
