...
use crate::entities::db::canvas::{CanvasFence, CanvasId, CanvasUiPosition, FindCanvasById};
use crate::entities::db::certificate::ListCertificatesBySnis;
...
impl NodeService {
    async fn views_for(
        &self,
        topology: &CanvasTopology,
    ) -> Result<Vec<crate::entities::db::view::ServerConfigViewEntity>, OrchestrationError> {
        Ok(self
            .db
    ...
    async fn process(&self, input: ReplaceNodeSpec) -> Result<Self::Output, Self::Error> {
        ...
        if input.spec.takes_bundles() {
            let primary = universal::Primary {
                edits,
                batch: crate::entities::db::batch::ApplyTopologyBatch {
                    canvas: Some(canvas.clone()),
                    set_specs: vec![crate::entities::db::batch::SpecUpdate {
                        node: input.node.clone(),
                        spec: input.spec.clone(),
                    }],
                    reshape: vec![crate::entities::db::batch::PortReshape {
                        node: input.node.clone(),
                        ports: ports.clone(),
                    }],
                    ..Default::default()
